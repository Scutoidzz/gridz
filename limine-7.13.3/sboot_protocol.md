# The sboot Boot Protocol

The sboot boot protocol is a modern, portable, featureful, and extensible boot
protocol.

This file serves as the official centralised collection of features that
the sboot boot protocol is comprised of. Other bootloaders may support extra
unofficial features, but it is strongly recommended to avoid fragmentation
and submit new features by opening a pull request to the sboot repository.

The [sboot.h](/sboot.h) file provides an implementation of all the
...
<<<<<<<< HEAD:limine-7.13.3/PROTOCOL.md
The Limine boot protocol comes in several base revisions; so far, 3
base revisions are specified: 0, 1, and 2.
========
The sboot boot protocol comes in several base revisions; so far only 2
base revisions are specified: 0 and 1.
>>>>>>>> 3d903c52bbc73a6de34a99a0c8ea0d72cca67161:limine-7.13.3/sboot_protocol.md

Base protocol revisions change certain behaviours of the sboot boot protocol
...
```c
struct sboot_example_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_example_response *response;
...
========
to boot an executable with multiple of the same request IDs. Alternatively, it is possible to provide a list of requests explicitly via an executable file section. See "sboot Requests Section".
>>>>>>>> 3d903c52bbc73a6de34a99a0c8ea0d72cca67161:limine-7.13.3/sboot_protocol.md
...
This is all there is to features. For a list of official sboot features, read
...
if the executable kernel file contains a `.sboot_reqs` section, the bootloader
...
struct sboot_bootloader_info_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_bootloader_info_response *response;
};
```
...
struct sboot_stack_size_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_stack_size_response *response;
    uint64_t stack_size;
};
```
...
struct sboot_hhdm_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_hhdm_response *response;
};
```
...
struct sboot_terminal_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_terminal_response *response;
    sboot_terminal_callback callback;
};
```
...
Response:
```c
typedef void (*sboot_terminal_write)(struct sboot_terminal *terminal, const char *string, uint64_t length);

struct sboot_terminal_response {
    uint64_t revision;
    uint64_t terminal_count;
    struct sboot_terminal **terminals;
    sboot_terminal_write write;
};
```

* `terminal_count` - How many terminals are present.
* `terminals` - Pointer to an array of `terminal_count` pointers to
`struct sboot_terminal` structures.
...
The `terminal` parameter points to the `struct sboot_terminal` structure to
...
```c
struct sboot_terminal {
    uint64_t columns;
    uint64_t rows;
    struct sboot_framebuffer *framebuffer;
};
```
...
```c
void callback(struct sboot_terminal *terminal, uint64_t type, uint64_t, uint64_t, uint64_t);
```

The `terminal` argument is a pointer to the sboot terminal structure which
...
* `sboot_TERMINAL_CB_DEC` - (type value: `10`)
...
* `sboot_TERMINAL_CB_BELL` - (type value: `20`)
...
* `sboot_TERMINAL_CB_PRIVATE_ID` - (type value: `30`)
...
* `sboot_TERMINAL_CB_STATUS_REPORT` - (type value `40`)
...
* `sboot_TERMINAL_CB_POS_REPORT` - (type value `50`)
...
* `sboot_TERMINAL_CB_KBD_LEDS` - (type value `60`)
...
* `sboot_TERMINAL_CB_MODE` - (type value: `70`)
...
* `sboot_TERMINAL_CB_LINUX` - (type value `80`)
...
overwritten, but sboot guarantees that the "visible" portion is going to
...
struct sboot_framebuffer_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_framebuffer_response *response;
};
```
...
struct sboot_framebuffer {
    ...
    /* Response revision 1 */
    uint64_t mode_count;
    struct sboot_video_mode **modes;
};
```

`modes` is an array of `mode_count` pointers to `struct sboot_video_mode` describing the
...
struct sboot_paging_mode_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_paging_mode_response *response;
    uint64_t mode;
...
respectively.
========
The `sboot_PAGING_MODE_DEFAULT` macro is provided by all architectures to select
...
The default mode (when this request is not provided) is `sboot_PAGING_MODE_X86_64_4LVL`.
...
The default mode (when this request is not provided) is `sboot_PAGING_MODE_AARCH64_4LVL`.
...
The default mode (when this request is not provided) is `sboot_PAGING_MODE_RISCV_SV48`.
...
Request:
```c
struct sboot_5_level_paging_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_5_level_paging_response *response;
};
```

Response:
```c
struct sboot_5_level_paging_response {
    uint64_t revision;
};
```
...
Request:
```c
struct sboot_smp_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_smp_response *response;
    uint64_t flags;
};
```
...
```c
struct sboot_smp_response {
    uint64_t revision;
    uint32_t flags;
    uint32_t bsp_lapic_id;
    uint64_t cpu_count;
    struct sboot_smp_info **cpus;
};
```
...
`struct sboot_smp_info` structures.
...
```c
struct sboot_smp_info;

typedef void (*sboot_goto_address)(struct sboot_smp_info *);

struct sboot_smp_info {
    uint32_t processor_id;
    uint32_t lapic_id;
    uint64_t reserved;
    sboot_goto_address goto_address;
...
`struct sboot_smp_info` structure of the CPU is passed in `RDI`. Other than
...
* `goto_address` - An atomic write to this field causes the parked CPU to
jump to the written address, on a 64KiB (or Stack Size Request size) stack. A pointer to the
`struct sboot_smp_info` structure of the CPU is passed in `X0`. Other than
that, the CPU state will be the same as described for the bootstrap
processor. This field is unused for the structure describing the bootstrap
processor.
* `extra_argument` - A free for use field.
...
* `goto_address` - An atomic write to this field causes the parked CPU to
jump to the written address, on a 64KiB (or Stack Size Request size) stack. A pointer to the
`struct sboot_smp_info` structure of the CPU is passed in `x10`(`a0`). Other than
that, the CPU state will be the same as described for the bootstrap
processor. This field is unused for the structure describing the bootstrap
processor.
* `extra_argument` - A free for use field.
...
struct sboot_memmap_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_memmap_response *response;
};
```
...
struct sboot_entry_point_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_entry_point_response *response;
    sboot_entry_point entry;
};
```
...
struct sboot_kernel_file_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_kernel_file_response *response;
};
```
...
struct sboot_kernel_file_response {
    uint64_t revision;
    struct sboot_file *kernel_file;
};
```

* `kernel_file` - Pointer to the `struct sboot_file` structure (see below)
for the kernel file.
...
struct sboot_module_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_module_response *response;

    /* Request revision 1 */
    uint64_t internal_module_count;
    struct sboot_internal_module **internal_modules;
};
```
...
`struct sboot_internal_module` structures.

Note: Internal modules are honoured if the module response has revision >= 1.

As part of `struct sboot_internal_module`:
...
========
  - `sboot_INTERNAL_MODULE_REQUIRED`: Fail if the requested module is not found.
>>>>>>>> 3d903c52bbc73a6de34a99a0c8ea0d72cca67161:limine-7.13.3/sboot_protocol.md

Internal sboot modules are guaranteed to be loaded *before* user-specified
...
* `revision` - Revision of the `struct sboot_file` structure.
...
struct sboot_rsdp_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_rsdp_response *response;
};
```
...
struct sboot_smbios_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_smbios_response *response;
};
```
...
struct sboot_efi_system_table_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_efi_system_table_response *response;
};
```
...
struct sboot_efi_memmap_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_efi_memmap_response *response;
};
```
...
struct sboot_boot_time_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_boot_time_response *response;
};
```
...
struct sboot_kernel_address_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_kernel_address_response *response;
};
```
...
struct sboot_dtb_request {
    uint64_t id[4];
    uint64_t revision;
    struct sboot_dtb_response *response;
};
```
