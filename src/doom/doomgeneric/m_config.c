...
static default_t extra_defaults_list[] =
{
    ...
    //!
    // If non-zero, long-clicking a mouse button acts like pressing
...
static void SaveDefaultCollection(default_collection_t *collection)
{
    ...
    for (i=0 ; i<collection->numdefaults ; i++)
    {
        ...
        switch (defaults[i].type) 
        {
            ...
            case DEFAULT_FLOAT:
                fprintf(f, "%f", * (long *) defaults[i].location);
                break;
...
static void SetVariable(default_t *def, char *value)
{
    ...
    switch (def->type)
    {
        ...
        case DEFAULT_FLOAT:
            * (long *) def->location = (long) atof(value);
            break;
    }
}
...
long M_GetFloatVariable(char *name)
{
    default_t *variable;
