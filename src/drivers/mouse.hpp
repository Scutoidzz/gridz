...
volatile uint8_t mouse_buttons = 0;   // bit0=left, bit1=right, bit2=middle
...
static inline void mouse_handle_irq() {
    ...
    // If buffer is empty or data is from keyboard, discard and EOI.
    if (!(status & 0x01)) {
        outb(0x20, 0x20); outb(0xA0, 0x20); return;
    }
    if (!(status & 0x20)) {
        (void)inb(0x60);  // drain keyboard byte to clear buffer
        outb(0x20, 0x20); outb(0xA0, 0x20); return;
    }
    ...
    switch (mouse_cycle) {
        case 0:
            if (!(data & 0x08)) break;   // bit 3 must always be set
            if (data & 0xC0)   break;   // X/Y overflow — discard packet
            mouse_packet[0] = data;
            mouse_cycle = 1;
            break;
        case 1:
